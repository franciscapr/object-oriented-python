...
class Bank():
    # Guardamos toda la informaciòn relevante para nuestras variables de instancia
    ...
    # Es llamado desde varios mètodos para solicitar un nùmero de cuenta e intentar verificar el nùmero
    def askForValidAccountNumber(self):
        accountNumber = input('What is your account number? ')
        # Bloque try/except --> nos aseguramos de que el nùmero sea entero
        try:
            accountNumber = int(accountNumber)
        # Generamos la excepciòn en caso de que el nùmero no sea entero
        except ValueError:
        # Lanzamos una excepciòn personalizada AbortTransaction
    ...
    def getUsersAccount(self):
        # Llamamos al mètodo anterior
        accountNumber = self.askForValidAccountNumber()
        # Usamos el nùmero de cuenta para encontrar el objeto Account adecuado
